...
## [2026-01-16] - Deployment 9567f001

### Changes
- **Commit**: 9567f001
- **Message**: fix(LTS): Remove autoheal service, rely on native Docker restart policies

BREAKING CHANGE: Autoheal container removed in favor of native self-healing

Rationale:
- Minimize dependencies for LTS stability
- Native Docker restart policies (restart: unless-stopped) are 100% reliable
- Reduces memory overhead and eliminates potential failure point
- Self-healing still provided by:
  1. Native Docker restart policies (all 15 services)
  2. ark-manager.sh heal command for manual/automated recovery
  3. Ralph Loop for comprehensive health monitoring

This aligns with LTS philosophy: 'Software should be robust enough to run in the dark'
by relying on kernel-level restart policies rather than external agents.
...
## [2026-01-16] - Deployment ff7e6c8f

### Changes
- **Commit**: ff7e6c8f
- **Message**: feat(Industrial Pivot): Transform ARK from homelab to sovereign OT platform

BREAKING CHANGE: Complete narrative shift from nomadic homelab to industrial infrastructure

Industrial Pivot Changes:
- README.md: Complete rewrite with industrial/military tone
  - Removed all casual language, emojis, 'nomad' references
  - Repositioned as 'Sovereign Operational Technology (OT) for DDIL environments'
  - Added CONOPS link, operational scenarios (DLE, mining, aerospace)
  - Emphasized 'Dark Site' problem, autonomous recovery, zero human intervention
- CONOPS.md: Created authoritative Concept of Operations document
  - Executive summary for industrial applications
  - Operational scenarios (DLE, mining, space analogs)
  - Technical specifications and validation data
  - Mobile Node Alpha validation narrative

New Narrative:
- Problem: Industrial operations fail when connectivity severed
- Solution: Sovereign OT layer for extreme edge deployments
- Applications: Mining, DLE, aerospace, subsurface operations
- Validation: 12+ months field testing via Mobile Node Alpha

This transforms ARK from personal project to industrial platform positioning.
